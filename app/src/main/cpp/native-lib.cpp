...
SerialPort_open(JNIEnv *env, jobject thiz, jstring path,jint baudrate) {
...
SerialPort_close(JNIEnv * env, jobject thiz)
{
...
extern "C"
JNIEXPORT jint JNICALL
SerialPort_send(JNIEnv * env, jobject thiz, jstring buf, jint buf_len) {
    jboolean iscopy;
    const char *str_utf = env->GetStringUTFChars(buf, &iscopy);
    if (!str_utf || buf_len<=0)
    {
